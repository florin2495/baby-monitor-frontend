...
@Component({
  ...
  styles: [`
    ...
    .header-logo {
      ...
      color: #8B5CF6; letter-spacing: -0.3px;
    }
    .baby-pills {
    ...
    .baby-pill--add {
      ...
      background: #FFF8F5; color: #C4A99A; border: 1.5px dashed #F5E6DE;
      font-size: 16px; font-weight: 700; text-decoration: none;
    }
    .shell-content { flex: 1; overflow-y: auto; overflow-x: hidden; min-height: 0; }
    .content-inner {
      padding: 16px 20px;
      padding-left: max(20px, env(safe-area-inset-left, 0px));
      padding-right: max(20px, env(safe-area-inset-right, 0px));
      padding-bottom: 24px; max-width: 428px; margin: 0 auto;
    }
    .tab-bar {
    ...
    .tab-item {
      ...
      padding: 2px 8px; text-decoration: none; color: #C4A99A;
      transition: color 0.2s ease; min-width: 48px;
    }
    .tab-item.tab-active { color: #8B5CF6; }
...
export class ShellComponent implements OnInit {
  ...
  ngOnInit() { this.ctx.loadBabies(); }

  selectBaby(index: number) {
    this.ctx.switchBaby(index);
